...
async def start_order_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user.user_type == "Исполнитель":
        orders = db.query(Order).filter(Order.executor_id == user_id, Order.status == "В работе").all()
    else:
        orders = db.query(Order).filter(Order.customer_id == user_id, Order.status != "Завершенный").all()
...
async def confirm_completion(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if order == "В работе":
        await update_order_status(db,order_to_change.id, "Завершенный")
    db.close()
...
async def cancel_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not order or (user.user_type == "Исполнитель" and order.executor_id != user_id) or (user.user_type == "Заказчик" and order.customer_id != user_id):
